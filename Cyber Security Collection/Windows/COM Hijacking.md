...
Многие приложения и скрипты обращаются к COM-объектам не напрямую по CLSID, а по их текстовым идентификаторам - ProgID (например, `WScript.Shell`). 

`ProgID` (Programmatic Identifier) представляет собой удобочитаемый псевдоним для CLSID. Когда приложение запрашивает объект по ProgID, система сначала преобразует этот текстовый идентификатор в соответствующий CLSID, после чего создание объекта идет по стандартному пути. Важно понимать, что ProgID может включать номер версии (например, `Word.Application.16`), и эта привязка жестко задана в коде приложения.
`VersionIndependentProgID` решает проблему версионности. Это общий идентификатор (например, `Word.Application`), который всегда указывает на последнюю или основную версию COM-объекта, установленную в системе. Это позволяет разработчикам писать код, который не сломается после обновления версии компонента.

Техника атаки заключается в перенаправлении этих запросов. Злоумышленник создает свой CLSID и регистрирует его под нужным ProgID. Когда приложение запрашивает объект по этому ProgID, система возвращает CLSID злоумышленника.
